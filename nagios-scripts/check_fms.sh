...
print_help() {
	...
	print_usage
	echo ""
	echo "FMS Service monitor plugin for Nagios"
	echo ""
...
{
	PROCESS="0"
	if [ `ps -ef | grep fms | grep -v grep | grep -v check_fms | wc -l` -lt 4 ]; then 
		echo "FMS NOT OK - One or more processes not running, total we have 4 and more"
...
echo "FMS OK - 4 Process is running"
exitstatus=$STATE_OK
exit $exitstatus
