...
print_help() {
	...
	print_usage
	echo ""
	echo "Apache Service monitor plugin for Nagios"
	echo ""
...
{
	PROCESS="0"
	if [ `ps -ef | grep httpd | grep -v grep | wc -l` -lt 2 ]; then 
		echo "Apache NOT OK - One or more processes not running"
...
echo "Apache is OK -- HTTPD process running"
exitstatus=$STATE_OK
exit $exitstatus
